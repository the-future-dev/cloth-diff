...
class TransformerPrivilegedPolicy(BasePrivilegedPolicy):
    ...
    - compute_loss expects a batch containing keys:
      { 'image': Tensor[B,T,...], 'state': Tensor[B,T,Ds], 'action': Tensor[B,T,Da] }
      If 'state' is missing, only the image loss is used (for flexibility).
    - predict_action uses only the image observation. It accepts either
      {'image': ...} or a generic {'obs': ...} for convenience.
    
    The obs_encoder architecture is supported through encoder configuration for each branch.
    """
    ...
    def __init__(self,
                 ...
                 image_encoder_type: str = "identity",
                 image_encoder_kwargs: Optional[Dict] = None,
                 # State encoder configuration
                 state_encoder_type: str = "identity", 
                 state_encoder_kwargs: Optional[Dict] = None,
        ...
        self.image_branch = TransformerImagePolicy(
            ...
            encoder_kwargs=image_encoder_kwargs,
            d_model=d_model,
            nhead=nhead,
        ...
        self.lowdim_branch = TransformerLowDimPolicy(
            ...
            encoder_kwargs=state_encoder_kwargs,
            d_model=d_model,
            nhead=nhead,
            num_layers=num_layers,
            ff_dim=ff_dim,
    ...
    def set_normalizer(self, normalizer):
        # Optionally propagate a shared normalizer
        self.normalizer = normalizer
        try:
            self.image_branch.set_normalizer(normalizer)
        except Exception:
            pass
        try:
            self.lowdim_branch.set_normalizer(normalizer)
        except Exception:
            pass
